      ...
      try
      {
        ...
        foreach (XmlNode codeNode in memberNode.SelectNodes("codeblock"))
        {
          ...
          if (!cb.IsPresentIn(existingCodeblocks))
          {
            ...
            if (mcb.EndLine != 0 && cb.Body.Trim() != string.Empty)
            {
              block = Sprocs.GetOrCreateMemberCodeBlock(mcb.MemberId, mcb.CodeHash, mcb.Version, mcb.FileName.SanetizeFilePath(), (int)mcb.StartLine, (int)mcb.StartColumn, (int)mcb.EndLine, (int)mcb.EndColumn);
            }
          }
          if (block == null)
          {
          ...
          if (block != null && codeNode.SelectSingleNode("refs") != null)
          {
